"""
Populate SPECTRA with realistic demo data for a presentation.

Creates:
  - Pipeline "CorpBot-Internal" (LangChain, 3 agents)
  - Completed audit with critical risk score
  - Execution run with blast radius + persistence evidence
  - 26 execution events showing full attack chain

Usage:
    python scripts/demo_seed.py            # idempotent — safe to run twice
    python scripts/demo_seed.py --reset    # deletes demo records and re-inserts
"""
...
RUN_ID      = AUDIT_ID   # same UUID so /audits/:id navigates to correct run

# Base timestamp: yesterday 14:30 UTC
...
# (sequence, event_type, node_id, payload_sent, response_received, classification, severity, duration_ms, offset_s, metadata)
EVENTS = [
...
async def delete_demo(db):
    ...
    await db.commit()
    print("Existing demo records deleted.")
...
async def main(reset: bool) -> None:
    ...
    async with Session() as db:
        ...
        if existing:
            ...
            db.add(pipeline)
            await db.flush()
            print(f"  + Pipeline: {pipeline.name} [{pipeline.framework}]")

        # ── Audit ─────────────────────────────────────────────────────────────
        existing = (await db.execute(select(Audit).where(Audit.id == AUDIT_ID))).scalar_one_or_none()
        if existing:
            print(f"Audit '{AUDIT_ID}' already exists — skipping.")
        else:
            audit = Audit(**AUDIT_DATA)
            db.add(audit)
            await db.flush()
            print(f"  + Audit: {audit.name} [status={audit.status}]")

        # ── ExecutionRun ──────────────────────────────────────────────────────
        existing = (await db.execute(select(ExecutionRun).where(ExecutionRun.id == RUN_ID))).scalar_one_or_none()
        if existing:
            print(f"ExecutionRun '{RUN_ID}' already exists — skipping.")
        else:
            run = ExecutionRun(**RUN_DATA)
            db.add(run)
            await db.flush()
            print(f"  + ExecutionRun: blast={run.blast_radius_score} persistence={run.persistence_detected}")
        ...
        if first_event:
            print(f"ExecutionEvents for run '{RUN_ID}' already exist — skipping.")
        else:
            events_added = 0
            for (seq, etype, node, payload, response, classification, severity, duration, offset, meta) in EVENTS:
                ev = ExecutionEvent(
                    id=str(uuid.uuid4()),
                    run_id=RUN_ID,
                    sequence=seq,
                ...
                events_added += 1
            await db.flush()
            print(f"  + {events_added} ExecutionEvents inserted")
    ...
    print()
    print("═" * 60)
    print("  SPECTRA demo data ready")
    print("═" * 60)
    print(f"  Pipeline  : CorpBot-Internal (LangChain)")
    print(f"  Audit     : Auditoría Q2 2026 — status: completed")
    print(f"  Risk      : CRITICAL (blast radius 81/100)")
    print(f"  Findings  : 4 (2× critical, 2× high)")
    print(f"  Persistence: detected across 3 sessions")
    print(f"  Events    : 26")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reset",
        action="store_true",
        help="Delete existing demo records before re-inserting",
    )
